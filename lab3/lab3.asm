...
.data
    ...
    NUMS_SIZE        equ 5h  ; количество вводимых чисел
    ...
    ; буферные переменные
    tmp_num    db 0, 5 dup(?)        ; переменная для промежуточного сохранения двоично-десятичного представления числа
                                     ; первый байт tmp_num - знаковый
    in_str     db 7, ?, 6 dup (?)    ; буфер ввода
    out_str    db 6 dup (' '),'$'    ; буфер вывода

    ; массивы, первый байт в массиве для учета количества записанных чисел
    pos_array  db 0, 5 dup(5 dup(?)) ; массив для пяти модулей положительных двоично-десятичных чисел размером 5 байт
    neg_array  db 0, 5 dup(5 dup(?)) ; массив для пяти модулей отрицательных двоично-десятичных чисел размером 5 байт
...
start:
    ...
    ; инициализация указателей на массивы (используется в add_value proc)
    ...
    .correct_input:
        call to_decimal  ; преобразование in_str к двоично-десятичному виду в tmp_num (с незначащими нулями)
